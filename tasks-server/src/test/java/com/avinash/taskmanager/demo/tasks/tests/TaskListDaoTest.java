...
public class TaskListDaoTest extends ApplicationTests {
	...
	public void setUp() {
		...
		Mockito.when(userDao.findByUserId(user.getUserId())).thenReturn(user);
	}
	...
	public void whenFindByUser_userIdAndTrashedFalse_thenReturnTaskListEntity() {
		...
		Page<TaskListEntity> pageTaskListEntity = taskListDao.findByUser_userIdAndTrashedFalse(userEntity.getUserId(),
				new PageRequest(0, 1));
